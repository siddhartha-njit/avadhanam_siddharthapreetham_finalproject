...
RANDOM_STATE = 42
KFOLDS = 10
SMOTE_TARGET_RATIO = 0.4286  # ~70/30 class split (minority/majority)
...
def rebalance_with_smote(X, y, sampling_ratio=SMOTE_TARGET_RATIO):
    if sampling_ratio <= 0:
        return X, y
    smote = SMOTE(
...
def run_model_evaluation(df, n_splits=KFOLDS):
    ...
    for tr_idx, te_idx in skf.split(X_raw, y):
        ...
        Xtr_bal, ytr_bal = rebalance_with_smote(Xtr, ytr)

        rf, yhat_rf, yprob_rf = train_predict_rf(Xtr_bal, ytr_bal, Xte)
        ...
        svm, yhat_svm, yprob_svm = train_predict_svm(Xtr_bal, ytr_bal, Xte)
        ...
        cnn, yhat_cnn, yprob_cnn = train_predict_conv1d(Xtr_bal, ytr_bal, Xte)
...
def main():
    ...
    y_full = df[TARGET].values.astype(int)
    X_full_bal, y_full_bal = rebalance_with_smote(X_full, y_full)
    rf_full = RandomForestClassifier(
        n_estimators=400, min_samples_leaf=2, max_features="sqrt",
        class_weight="balanced", random_state=RANDOM_STATE, n_jobs=-1
    )
    rf_full.fit(X_full_bal, y_full_bal)
